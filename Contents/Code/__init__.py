...
class FilmWebStandaloneAgent(Agent.Movies):
	...
	def update(self, metadata, media, lang): 
		...
		overview = ""       
		try:
			overview = info_filmweb_result.xpath(".//p[contains(@property, 'v:summary')]")[0].text      
		...
		#RATING
		try:
			rate = info_filmweb_result.xpath(".//div[contains(@class, 'ratingInfo')]")[0].xpath(".//span[@property='v:average']")[0].text;
			rate = rate[1:].replace(',','.')
		...
		for actors in info_filmweb_result.xpath(".//table[contains(@class, 'filmCast')]//tr[starts-with(@id, 'role')]"):
			try:
				...
				role.role = re.sub(' +', ' ', actors.xpath(".//td")[3].text_content())
				role.actor = actors.xpath(".//td/a")[1].text_content()       
				Log("ACTOR: " + role.actor + " AS " + role.role);        
			except:
				pass
